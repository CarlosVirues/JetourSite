...
// --- Noticias (sección /noticias, reconectada a Sanity el 2026-09-21) ---
//
// Filtro `defined(mainImage.asset)` en las 4 queries: hay 18 documentos newsArticle
// "zombie" en production (migrados desde el lib/data-site.js hardcoded de una prueba
// anterior) con excerpt Lorem ipsum y SIN imagen — nunca pasaron por Studio de verdad.
// En vez de borrarlos, se filtran acá: como mainImage es obligatorio en el schema para
// cualquier noticia real, este filtro los excluye automáticamente sin tocar datos en
// Sanity. Cris/Carlos pueden limpiarlos desde Studio cuando quieran, sin apuro.
...
function formatNewsDate(iso) {
  ...
  return `${MESES_ES[d.getUTCMonth()]} ${d.getUTCDate()}, ${d.getUTCFullYear()}`;
}
...
function mapNewsCard(article) {
  return { ...article, date: formatNewsDate(article.publishedAt) };
}

export async function getFeaturedNews() {
  try {
    const data = await client.fetch(`
      *[_type == "newsArticle" && featured == true && defined(mainImage.asset)]
        | order(publishedAt desc) ${NEWS_CARD_PROJECTION}
    `);
    return (data || []).map(mapNewsCard);
  } catch (error) {
...
export async function getAllNews() {
  try {
    const data = await client.fetch(`
      *[_type == "newsArticle" && defined(mainImage.asset)]
        | order(publishedAt desc) ${NEWS_CARD_PROJECTION}
    `);
    return (data || []).map(mapNewsCard);
  } catch (error) {
...
export async function getNewsCategories() {
  try {
    // slug != "todo": categoría "zombie" de la migración de placeholders (ver nota arriba),
    // redundante con el filtro "Todo" que ya arma la UI por su cuenta.
    const data = await client.fetch(`
      *[_type == "newsCategory" && slug.current != "todo"]
...
export async function getNewsArticleBySlug(slug) {
  try {
    const data = await client.fetch(
      `*[_type == "newsArticle" && slug.current == $slug && defined(mainImage.asset)][0]{
        "id": _id,
        title,
        "slug": slug.current,
        excerpt,
    ...
    if (!data) return null;
    return { ...data, date: formatNewsDate(data.publishedAt) };
  } catch (error) {
...
export async function getRelatedNews(currentSlug, categorySlug, limit = 3) {
  try {
    const data = await client.fetch(
      `*[_type == "newsArticle" && slug.current != $currentSlug && defined(mainImage.asset)
          && category->slug.current == $categorySlug]
        | order(publishedAt desc) [0...$limit] ${NEWS_CARD_PROJECTION}`,
