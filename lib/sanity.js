...
export const client = isValidProjectId && dataset
  ? createClient({
      projectId,
      dataset,
      useCdn: false,
...
if (!client && typeof window === "undefined") {
  if (!projectId || !dataset) {
    ...
    console.warn(
      `[sanity] Cliente no inicializado: projectId "${projectId}" tiene formato inválido. ` +
      "Sanity requiere solo a-z, 0-9 y guiones. Revisar la env var (puede tener espacios/mayúsculas)."
    );
  }
}
