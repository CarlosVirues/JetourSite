...
export default function QuoteForm({ currentModel = null, source = null }) {
  ...
  // ⬇️ NUEVO: redirección “real” del lado cliente
  useEffect(() => {
    if (state?.success) {
  ...
  return (
    <motion.section variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="bg-black py-20">
      <div className="max-w-4xl mx-auto px-6 lg:px-12">
        {/* Headline */}
        <motion.div variants={itemVariants} className="text-center mb-12">
          <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-3xl lg:text-5xl font-bold text-white mb-6">
        ...
        {state.message && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            className={`p-4 mb-4 rounded-lg flex items-center gap-3 ${state.success ? "bg-green-500/10 border border-green-500 text-green-400" : "bg-red-500/10 border border-red-500 text-red-400"}`}>
        ...
        <motion.div variants={itemVariants} className="bg-black rounded-2xl p-8 lg:p-12">
          <form action={action} noValidate className="space-y-6" id="quote-form">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <motion.div variants={formFieldVariants} className="relative">
                <div className="flex items-center border-b border-blue-500 pb-2">
                  ...
                  <input type="text" name="nombre" defaultValue={state.values?.nombre || ""} placeholder="Nombre y apellido" className="flex-1 bg-transparent text-white placeholder-gray-400 focus:outline-none" required />
                </div>
                {state.errors?.nombre && (<motion.p className="text-red-400 text-sm mt-1 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{state.errors.nombre[0]}</motion.p>)}
              </motion.div>
              ...
              <motion.div variants={formFieldVariants} className="relative">
                <div className="flex items-center border-b border-blue-500 pb-2">
                  ...
                  <input type="tel" name="celular" defaultValue={state.values?.celular || ""} placeholder="Celular" className="flex-1 bg-transparent text-white placeholder-gray-400 focus:outline-none" required />
                </div>
                {state.errors?.celular && (<motion.p className="text-red-400 text-sm mt-1 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{state.errors.celular[0]}</motion.p>)}
              </motion.div>
              ...
              <motion.div variants={formFieldVariants} className="relative">
                <div className="flex items-center border-b border-blue-500 pb-2">
                  ...
                  <input type="email" name="mail" defaultValue={state.values?.mail || ""} placeholder="Mail" className="flex-1 bg-transparent text-white placeholder-gray-400 focus:outline-none" required />
                </div>
                {state.errors?.mail && (<motion.p className="text-red-400 text-sm mt-1 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{state.errors.mail[0]}</motion.p>)}
              </motion.div>
              ...
              <motion.div variants={formFieldVariants} className="relative">
                <div className="flex items-center border-b border-blue-500 pb-2">
                  ...
                  <input type="text" name="cedula" defaultValue={state.values?.cedula || ""} placeholder="Cédula" className="flex-1 bg-transparent text-white placeholder-gray-400 focus:outline-none" required />
                </div>
                {state.errors?.cedula && (<motion.p className="text-red-400 text-sm mt-1 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{state.errors.cedula[0]}</motion.p>)}
              </motion.div>
              ...
              <motion.div variants={formFieldVariants} className="relative md:col-span-2">
                <div className="flex items-center border-b border-blue-500 pb-2">
                  ...
                  <select key={state.values?.ciudad || ""} name="ciudad" defaultValue={state.values?.ciudad || ""} className="flex-1 bg-black text-white focus:outline-none" required>
                    <option value="">Selecciona el concesionario mas cercano</option>
                    {ciudades.map((city) => (
                ...
                {state.errors?.ciudad && (<motion.p className="text-red-400 text-sm mt-1 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{state.errors.ciudad[0]}</motion.p>)}
              </motion.div>
            </div>
            ...
            {/* Modelos (sin cambios) */}
            {/* ...tu render de modelos exactamente como lo tienes... */}
